...
namespace wholegraph_torch {
  ...
  torch_tensor_check_dtype_is_index(input_nodes, "weighted_sample_without_replacement, input_nodes");
  ...
  auto wm_csr_weight_ptr_tensor = reinterpret_cast<wholememory_tensor_t>(csr_weight_ptr_wholememory_tensor_handle);
  ...
  auto* p_wm_csr_weight_ptr_tensor_desc = wholememory_tensor_get_tensor_description(wm_csr_weight_ptr_tensor);
  ...
  TORCH_CHECK(p_wm_csr_weight_ptr_tensor_desc->dim == 1, "csr_weight_ptr_wholememory_tensor_handle should be 1D WholeMemory Tensor.")
  ...
  TORCH_CHECK(p_wm_csr_weight_ptr_tensor_desc->dtype == WHOLEMEMORY_DT_FLOAT || WHOLEMEMORY_DT_DOUBLE, "csr_weight_ptr_wholememory_tensor_handle should be 1D WholeMemory Tensor.")
...
static auto registry =
  torch::RegisterOperators().op("wholegraph::weighted_sample_without_replacement", &wholegraph_torch::weighted_sample_without_replacement);