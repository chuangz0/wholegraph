...
torch::Tensor raft_pcg_generator_random(int64_t random_seed, int64_t subsequence, int64_t generated_random_number) {
...
torch::Tensor raft_pcg_generator_random_from_weight(int64_t random_seed, int64_t subsequence, torch::Tensor edge_weight, int64_t generated_random_number) {
  auto to = torch::TensorOptions().device(torch::kCPU).dtype(edge_weight.dtype()).requires_grad(false);
   torch::Tensor output = torch::empty({(long)(generated_random_number)}, to);
  TestPCGenerator rng((unsigned long long)random_seed, subsequence, 0);
  for (int64_t i = 0; i < generated_random_number; i++) {
