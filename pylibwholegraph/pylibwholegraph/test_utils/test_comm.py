...
def copy_host_1D_tensor_to_wholememory(wm_array, host_tensor, world_rank, world_size, wm_comm):

    local_tensor_cuda, local_start = wm_array.get_local_tensor(torch_import_from_dlpack, wmb.WholeMemoryMemoryLocation.MlDevice, world_rank)
    assert local_tensor_cuda.dim() == 1
    wm_array_size = wm_array.shape[0]

    local_start_ref = min(wmb.determine_partition_plan(wm_array_size, world_size) * world_rank, wm_array_size)
    local_end = min(wmb.determine_partition_plan(wm_array_size, world_size) * (world_rank + 1), wm_array_size)
...
def host_get_sample_offset_tensor(host_csr_row_ptr, center_nodes, max_sample_count):
    center_nodes_count = center_nodes.size(0)
    output_sample_offset_tensor = torch.empty((center_nodes_count + 1,), dtype = torch.int32)
