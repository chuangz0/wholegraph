...
def create_node_claffication_datasets(pickle_data_filename: str):
    ...
    train_dataset = list(list(zip(train_data["idx"], train_data["label"].astype(np.int64))))
    valid_dataset = list(list(zip(valid_data["idx"], valid_data["label"].astype(np.int64))))
    test_dataset = list(list(zip(test_data["idx"], test_data["label"].astype(np.int64))))

    return NodeClassificationDataset(train_dataset),\
...
def get_train_dataloader(train_dataset,
    ...
    train_sampler = torch.utils.data.distributed.DistributedSampler(
        train_dataset, num_replicas=num_replicas, rank=replica_id, shuffle=True, drop_last=True,
    )
...
def get_valid_test_dataloader(valid_test_dataset,
                              batch_size: int,
                              *,
                              num_workers: int = 0):
