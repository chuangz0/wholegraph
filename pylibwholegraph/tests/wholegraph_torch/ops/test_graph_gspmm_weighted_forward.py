...
def host_gspmm_weighted_forward(csr_row_ptr_tensor, csr_col_ptr_tensor, edge_weight_tensor, feature_tensor):
    ...
    output_feature_tensor = torch.zeros((row_num, num_head, feature_dim), dtype = feature_tensor.dtype)
        ...
        for head_id in range(num_head):
            for feature_id in range(feature_dim):
                for j in range(start, end):
                    ...
                    output_feature_tensor[row_id][head_id][feature_id] += edge_weight_tensor[j][head_id] * feature_tensor[col_id][head_id][feature_id]
...
def routine_func(**kwargs):
    ...
    output_feature_tensor_cuda = wg_ops.gspmm_weighted_forward(csr_row_ptr_tensor_cuda, csr_col_ptr_tensor_cuda, edge_weight_tensor_cuda, feature_tensor_cuda)
    output_feature_tensor = output_feature_tensor_cuda.cpu()
    output_feature_tensor_ref = host_gspmm_weighted_forward(csr_row_ptr_tensor, csr_col_ptr_tensor, edge_weight_tensor, feature_tensor)
...
def test_gspmm_weighted_forward(target_node_count, neighbor_node_count, edge_num, feature_dtype, feature_dim, num_head):
    gpu_count = torch.cuda.device_count()
    assert gpu_count > 0
    routine_func(target_node_count=target_node_count, neighbor_node_count=neighbor_node_count, edge_num = edge_num, feature_dtype=feature_dtype, feature_dim = feature_dim, num_head=num_head)
