...
def host_spadd_gat_forward(csr_row_ptr_tensor, csr_col_ptr_tensor, edge_weight_left_tensor, edge_weight_right_tensor):
    ...
    output_score_tensor = torch.empty((csr_col_ptr_tensor.shape[0], num_head), dtype = edge_weight_left_tensor.dtype)
    for row_id in range(row_num):
        ...
        for j in range(start, end):
            ...
            for head_id in range(num_head):
                output_score_tensor[j][head_id] = edge_weight_left_tensor[row_id][head_id] + edge_weight_right_tensor[col_id][head_id]
...
def routine_func(**kwargs):
    ...
    edge_weight_left_tensor = torch.rand((target_node_count, num_head), dtype = feature_dtype)
    edge_weight_right_tensor = torch.rand((neighbor_node_count, num_head), dtype = feature_dtype)
    ...
    output_score_tensor_cuda = wg_ops.spadd_gat_forward(csr_row_ptr_tensor_cuda, csr_col_ptr_tensor_cuda, edge_weight_left_tensor_cuda, edge_weight_right_tensor_cuda)
    output_score_tensor = output_score_tensor_cuda.cpu()
    output_score_tensor_ref = host_spadd_gat_forward(csr_row_ptr_tensor, csr_col_ptr_tensor, edge_weight_left_tensor, edge_weight_right_tensor)
...
def test_spadd_gat_forward(target_node_count, neighbor_node_count, edge_num, feature_dtype, num_head):
