...
def host_edge_weight_softmax_backward(csr_row_ptr_tensor, edge_weight_tensor, grad_edge_weight_softmax_tensor):
    row_num = csr_row_ptr_tensor.shape[0] - 1
    num_head = edge_weight_tensor.shape[1]
    output_grad_edge_weight_tensor = torch.empty((edge_weight_tensor.shape[0], edge_weight_tensor.shape[1]), dtype = edge_weight_tensor.dtype)
...
def routine_func(**kwargs):
    ...
    csr_row_ptr_tensor, _, _ = gen_csr_graph(target_node_count, edge_num, neighbor_node_count, csr_row_dtype=torch.int32, csr_col_dtype=torch.int32)
    edge_weight_tensor = torch.rand((edge_num, num_head), dtype = feature_dtype)
    grad_edge_weight_softmax_tensor = torch.rand((edge_num, num_head), dtype = feature_dtype)
    ...
    output_grad_edge_weight_tensor_cuda = wg_ops.edge_weight_softmax_backward(csr_row_ptr_tensor_cuda, edge_weight_tensor_cuda, grad_edge_weight_softmax_tensor_cuda)
    output_grad_edge_weight_tensor = output_grad_edge_weight_tensor_cuda.cpu()

    output_grad_edge_weight_tensor_ref = host_edge_weight_softmax_backward(csr_row_ptr_tensor, edge_weight_tensor, grad_edge_weight_softmax_tensor)
...
def test_edge_weight_softmax_backward(target_node_count, neighbor_node_count, edge_num, feature_dtype, num_head):
