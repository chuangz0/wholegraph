...
def host_add_csr_self_loop(csr_row_ptr_tensor, csr_col_ptr_tensor):
    ...
    output_csr_row_ptr_tensor = torch.empty((csr_row_ptr_tensor.shape[0],), dtype = csr_row_ptr_tensor.dtype)
    output_csr_col_ptr_tensor = torch.empty((edge_num + row_num,), dtype = csr_col_ptr_tensor.dtype)
...
def routine_func(**kwargs):
    ...
    output_csr_row_ptr_tensor_cuda, output_csr_col_ptr_tensor_cuda = wg_ops.add_csr_self_loop(csr_row_ptr_tensor_cuda, csr_col_ptr_tensor_cuda)
    output_csr_row_ptr_tensor = output_csr_row_ptr_tensor_cuda.cpu()
    output_csr_col_ptr_tensor = output_csr_col_ptr_tensor_cuda.cpu()
    output_csr_row_ptr_tensor_ref, output_csr_col_ptr_tensor_ref = host_add_csr_self_loop(csr_row_ptr_tensor, csr_col_ptr_tensor)
...
def test_add_csr_self_loop(target_node_count, neighbor_node_count, edge_num):
    ...
    assert gpu_count > 0
    routine_func(target_node_count=target_node_count, neighbor_node_count=neighbor_node_count, edge_num = edge_num)
