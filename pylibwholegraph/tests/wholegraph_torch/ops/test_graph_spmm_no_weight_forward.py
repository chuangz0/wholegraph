...
def host_general_spmm_forward(csr_row_ptr_tensor, csr_col_ptr_tensor, feature_tensor, aggregator):
...
def routine_func(**kwargs):
    ...
    output_feature_tensor_cuda = wg_ops.spmm_no_weight_forward(csr_row_ptr_tensor=csr_row_ptr_tensor_cuda, csr_col_ptr_tensor=csr_col_ptr_tensor_cuda, feature_tensor=feature_tensor_cuda, aggregator=aggregator)
    output_feature_tensor = output_feature_tensor_cuda.cpu()

    output_feature_tensor_ref = host_general_spmm_forward(csr_row_ptr_tensor, csr_col_ptr_tensor, feature_tensor, aggregator)
...
def test_spmm_no_weight_forward(target_node_count, neighbor_node_count, edge_num, feature_dim, feature_dtype, aggregator):
