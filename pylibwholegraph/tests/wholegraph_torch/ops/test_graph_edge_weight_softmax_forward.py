...
def host_edge_weight_softmax(csr_row_ptr_tensor, edge_weight_tensor):
    ...
    output_edge_weight_softmax_tensor = torch.empty((edge_weight_tensor.shape[0], edge_weight_tensor.shape[1]), dtype=edge_weight_tensor.dtype)
    for row_id in range(row_num):
        ...
        for head_id in range(num_head):
            ...
            for j in range(start, end):
                output_edge_weight_softmax_tensor[j][head_id] = math.exp(edge_weight_tensor[j][head_id] - max_value) / sum_exp
...
def routine_func(**kwargs):
    ...
    edge_weight_softmax_tensor_cuda = wg_ops.edge_weight_softmax_forward(csr_row_ptr_tensor_cuda, edge_weight_tensor_cuda)
    edge_weight_softmax_tensor = edge_weight_softmax_tensor_cuda.cpu()

    edge_weight_softmax_tensor_ref = host_edge_weight_softmax(csr_row_ptr_tensor, edge_weight_tensor)
...
def test_edge_weight_softmax_forward(target_node_count, neighbor_node_count, edge_num, feature_dtype, num_head):
