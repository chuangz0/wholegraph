...
def host_gspmm_weighted_backward(csr_row_ptr_tensor, csr_col_ptr_tensor, edge_weight_tensor, feature_tensor, grad_feature_tensor):
    ...
    output_grad_edge_weight_tensor = torch.zeros((edge_weight_tensor.shape[0], edge_weight_tensor.shape[1]), dtype = edge_weight_tensor.dtype)
    output_grad_feature_tensor = torch.zeros((feature_tensor.shape[0], feature_tensor.shape[1], feature_tensor.shape[2]), dtype = feature_tensor.dtype)
        ...
        for row_id in range(row_num):
            ...
            for feature_id in range(feature_dim):
                for j in range(start, end):
                    ...
                    output_grad_feature_tensor[col_id][head_id][feature_id] += edge_weight_tensor[j][head_id] * grad_feature_tensor[row_id][head_id][feature_id]
                    output_grad_edge_weight_tensor[j][head_id] += grad_feature_tensor[row_id][head_id][feature_id] * feature_tensor[col_id][head_id][feature_id]
...
def routine_func(**kwargs):
    ...
    feature_tensor = torch.rand((neighbor_node_count, num_head, feature_dim), dtype = feature_dtype)
    grad_feature_tensor = torch.rand((target_node_count, num_head, feature_dim), dtype = feature_dtype)
    ...
    output_tensor_cuda = wg_ops.gspmm_weighted_backward(csr_row_ptr_tensor_cuda, csr_col_ptr_tensor_cuda, edge_weight_tensor_cuda, feature_tensor_cuda, grad_feature_tensor_cuda, need_grad_edge_weight, need_grad_feature)
    output_tensors = None
    if output_tensor_cuda is not None:
        output_tensors = tuple(tensor.cpu() for tensor in output_tensor_cuda)
    output_grad_edge_weight_tensor_ref, output_grad_feature_tensor_ref = host_gspmm_weighted_backward(csr_row_ptr_tensor, csr_col_ptr_tensor, edge_weight_tensor, feature_tensor, grad_feature_tensor)
...
def test_gspmm_weighted_backward(target_node_count, neighbor_node_count, edge_num, feature_dtype, feature_dim, num_head, need_grad_edge_weight, need_grad_feature):
    gpu_count = torch.cuda.device_count()
    assert gpu_count > 0
    routine_func(target_node_count=target_node_count, neighbor_node_count=neighbor_node_count, edge_num = edge_num, feature_dtype=feature_dtype, feature_dim = feature_dim, num_head=num_head, need_grad_edge_weight=need_grad_edge_weight, need_grad_feature=need_grad_feature)
