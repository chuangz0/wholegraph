...
def load_routine_func(world_rank: int,
                      world_size: int,
    ...
    reference_local_tensor = cpu_embedding_tensor_base[rank_start_entry:rank_end_entry, :].cuda()
    ...
    for mt in [wmb.WholeMemoryMemoryType.MtContinuous, wmb.WholeMemoryMemoryType.MtChunked,
        ...
        for ml in [wmb.WholeMemoryMemoryLocation.MlHost, wmb.WholeMemoryMemoryLocation.MlDevice]:
            wholememory_root_tensor = wmb.create_wholememory_matrix(data_type,
                                                                    embedding_entry_count,
                                                                    ...
                                                                    mt,
                                                                    ml)
            wholememory_tensor = wholememory_root_tensor.get_sub_tensor([-1, storage_offset], [-1, -1])
            wholememory_tensor.from_filelist(file_list)
            local_tensor, local_offset = wholememory_tensor.get_local_tensor(torch_import_from_dlpack,
...
@pytest.mark.parametrize('embedding_entry_count', [1024 * 1024 *4 + 131, 1024 * 1024 * 6 - 127])
...
def test_wholememory_load(file_part_count,
                          embedding_entry_count,
                          embedding_dim,
                          embedding_stride,
                          storage_offset):
    if embedding_stride < storage_offset + embedding_dim:
        pytest.skip('Skipping due to embedding_stride, embedding_dim and storage_offset configuration not valid.')
    cpu_embedding_tensor_base = torch.randint(-1000000000, 1000000000, (embedding_entry_count, embedding_dim), dtype=torch.int, device='cpu')
    indices = sorted(random.sample(range(1, embedding_entry_count), file_part_count - 1))
    ...
    for i in range(file_part_count):
        splited_tensors[i].numpy().tofile('%s_part_%d_of_%d' % (file_name_prefix, i, file_part_count))

    cpu_embedding_tensor_base = cpu_embedding_tensor_base.share_memory_()

    load_routine_func_partial = partial(load_routine_func,
...
def store_routine_func(world_rank: int,
                       world_size: int,
                       file_name_prefix,
                       embedding_entry_count,
                       embedding_dim,
    ...
    root_data_tensor = torch.IntTensor(
        range(embedding_stride * local_root_offset,
    ...
    wholememory_tensor = wholememory_root_tensor.get_sub_tensor([-1, storage_offset], [-1, storage_offset + embedding_dim])
...
@pytest.mark.parametrize('embedding_stride', [16, 32, 64])
@pytest.mark.parametrize('storage_offset', [0, 3])
def test_wholememory_store(embedding_entry_count,
                           embedding_dim,
                           embedding_stride,
                           storage_offset):
    if embedding_stride < storage_offset + embedding_dim:
    ...
    store_routine_func_partial = partial(store_routine_func,
                                        file_name_prefix=file_name_prefix,
                                        embedding_entry_count=embedding_entry_count,
                                        embedding_dim=embedding_dim,
                                        embedding_stride=embedding_stride,
    ...
    for i in range(file_part_count):
        ...
        loaded_torch_tensor = torch.from_numpy(loaded_np_array).reshape((-1, embedding_dim))

        reference_tensor = torch.IntTensor(
            range(embedding_stride * embedding_entry_offset,
                  embedding_stride * (embedding_entry_offset + file_entry_count)))\
            .reshape((-1, embedding_stride))
        reference_tensor = reference_tensor[:, storage_offset: storage_offset + embedding_dim]
