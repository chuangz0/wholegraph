...
def array_test_case(wm_comm, dt, mt, ml, size):
    ...
    print('Rank=%d testing array size=%d dt=%s, mt=%s, ml=%s' % (world_rank, size, dt, mt, ml))
...
def matrix_test_case(wm_comm, dt, mt, ml, mat_size):
    world_rank = wm_comm.get_rank()
    world_size = wm_comm.get_size()
    print('Rank=%d testing matrix size=%s dt=%s, mt=%s, ml=%s' % (world_rank, mat_size, dt, mt, ml))
    wm_matrix = wmb.create_wholememory_matrix(dt, mat_size[0], mat_size[1], -1, wm_comm, mt, ml)
    wm_handle = wm_matrix.get_wholememory_handle()
    ...
    wm_sub_matrix = wm_matrix.get_sub_tensor([mat_size[0] // 3, mat_size[1] // 5], [-1, mat_size[1] // 5 * 3])
    ...
    assert wm_sub_matrix.storage_offset() == mat_size[1] // 5 + mat_size[0] // 3 * mat_size[1]
