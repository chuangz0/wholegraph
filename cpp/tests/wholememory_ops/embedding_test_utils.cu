...
class type_convertor<__half> {
 public:
  ...
  static __device__ __half convert_store_data(StoreTypeT data) {
...
void matrix_test_cast(void* dst,
  ...
  int block_count = static_cast<int>(raft::div_rounding_up_unsafe<int64_t>(row_count, rows_per_block));
  matrix_type_cast_kernel<SrcTypeT,
                          DstTypeT><<<block_count, threads_per_block, 0, stream>>>(static_cast<DstTypeT *>(dst),
...
REGISTER_DISPATCH_TWO_TYPES(FloatMatrixTestCast, matrix_test_cast, HALF_FLOAT_DOUBLE, HALF_FLOAT_DOUBLE)
...
void device_matrix_type_cast(void *dst,
  ...
  bool is_float_src = src_desc.dtype == WHOLEMEMORY_DT_HALF || src_desc.dtype == WHOLEMEMORY_DT_FLOAT
      || src_desc.dtype == WHOLEMEMORY_DT_DOUBLE || src_desc.dtype == WHOLEMEMORY_DT_BF16;
  bool is_float_dst = dst_desc.dtype == WHOLEMEMORY_DT_HALF || dst_desc.dtype == WHOLEMEMORY_DT_FLOAT
      || dst_desc.dtype == WHOLEMEMORY_DT_DOUBLE || dst_desc.dtype == WHOLEMEMORY_DT_BF16;
  EXPECT_EQ(is_float_dst, is_float_src);
  if (is_float_src) {
    DISPATCH_TWO_TYPES(src_desc.dtype, dst_desc.dtype, FloatMatrixTestCast,
                       dst, src, src_desc.sizes[1], src_desc.sizes[0],
                       dst_desc.stride, src_desc.stride, stream);
  } else {
    DISPATCH_TWO_TYPES(src_desc.dtype, dst_desc.dtype, IntMatrixTestCast,
                       dst, src, src_desc.sizes[1], src_desc.sizes[0],
                       dst_desc.stride, src_desc.stride, stream);
  }
}
...
void get_embedding_data(void *embedding_ptr,
  ...
  get_embedding_data_kernel<DataTypeT><<<block_count, block_size, 0, stream>>>(static_cast<DataTypeT*>(embedding_ptr),
                                                                               storage_offset,
                                                                               embedding_dim,
                                                                               embedding_stride,
                                                                               local_entry_start,
...
void device_random_init_local_embedding_table(wholememory_handle_t embedding_handle,
  ...
  int64_t embedding_entry_size = embedding_desc.stride * wholememory_dtype_get_element_size(embedding_desc.dtype);
  ...
  switch (embedding_desc.dtype) {
    case WHOLEMEMORY_DT_FLOAT:  {
      get_embedding_data<float>(local_embedding_ptr, embedding_desc, local_entry_start, local_entry_count, stream);
      break;
    }
    case WHOLEMEMORY_DT_HALF:   {
      get_embedding_data<__half>(local_embedding_ptr, embedding_desc, local_entry_start, local_entry_count, stream);
      break;
    }
    case WHOLEMEMORY_DT_DOUBLE: {
      get_embedding_data<double>(local_embedding_ptr, embedding_desc, local_entry_start, local_entry_count, stream);
      break;
    }
    case WHOLEMEMORY_DT_BF16:   {
      get_embedding_data<__nv_bfloat16>(local_embedding_ptr, embedding_desc, local_entry_start, local_entry_count, stream);
      break;
    }
    case WHOLEMEMORY_DT_INT:    {
      get_embedding_data<int>(local_embedding_ptr, embedding_desc, local_entry_start, local_entry_count, stream);
      break;
    }
    case WHOLEMEMORY_DT_INT64:  {
      get_embedding_data<int64_t>(local_embedding_ptr, embedding_desc, local_entry_start, local_entry_count, stream);
      break;
    }
    case WHOLEMEMORY_DT_INT16:  {
      get_embedding_data<int16_t>(local_embedding_ptr, embedding_desc, local_entry_start, local_entry_count, stream);
      break;
    }
    case WHOLEMEMORY_DT_INT8:   {
      get_embedding_data<int8_t>(local_embedding_ptr, embedding_desc, local_entry_start, local_entry_count, stream);
      break;
    }
    default: {
      FAIL();
...
void device_get_expected_embedding_temp_func(void* gen_buffer,
  ...
  device_get_expected_embedding_kernel<IndexT,
                                       GenTypeT><<<block_count, thread_count, 0, stream>>>(static_cast<GenTypeT *>(gen_buffer),
...
REGISTER_DISPATCH_TWO_TYPES(DeviceGetExpectedEmbedding, device_get_expected_embedding_temp_func, SINT3264, ALLSINT_ALLFLOAT)
...
void device_get_expected_embedding(void* output,
  ...
  if (embedding_dtype != output_desc.dtype) {
    ...
    gen_buffer =
        gen_buffer_tmh.device_malloc(wholememory_get_memory_element_count_from_matrix(&gen_desc), gen_desc.dtype);
  }
  DISPATCH_TWO_TYPES(indices_desc.dtype, gen_desc.dtype, DeviceGetExpectedEmbedding,
                     gen_buffer, gen_desc, indices, indices_desc, stream);
  if (embedding_dtype != output_desc.dtype) {
...
void host_random_init_indices(void *indices,
                              wholememory_array_description_t indices_desc) {
  EXPECT_TRUE(indices_desc.dtype == WHOLEMEMORY_DT_INT || indices_desc.dtype == WHOLEMEMORY_DT_INT64);
...
void host_check_embedding_same(void* host_embedding,
  ...
  for (int64_t row = 0; row < row_count; row++) {
    for (int64_t col = 0; col < col_count; col++) {
      ...
      if (element_size == 1) {
        embedding_data = load_hex_data<uint8_t>(host_embedding, row * embedding_desc.stride + embedding_desc.storage_offset + col);
        reference_data = load_hex_data<uint8_t>(host_reference, row * reference_desc.stride + reference_desc.storage_offset + col);
      } else if (element_size == 2) {
        embedding_data = load_hex_data<uint16_t>(host_embedding, row * embedding_desc.stride + embedding_desc.storage_offset + col);
        reference_data = load_hex_data<uint16_t>(host_reference, row * reference_desc.stride + reference_desc.storage_offset + col);
      } else if (element_size == 4) {
        embedding_data = load_hex_data<uint32_t>(host_embedding, row * embedding_desc.stride + embedding_desc.storage_offset + col);
        reference_data = load_hex_data<uint32_t>(host_reference, row * reference_desc.stride + reference_desc.storage_offset + col);
      } else {
        embedding_data = load_hex_data<uint64_t>(host_embedding, row * embedding_desc.stride + embedding_desc.storage_offset + col);
        reference_data = load_hex_data<uint64_t>(host_reference, row * reference_desc.stride + reference_desc.storage_offset + col);
      }
      if (embedding_data != reference_data) {
        if (diff_count < 10) {
          printf("row=%ld, col=%ld, got %lx (float %f), but should be %lx (float %f)\n",
                 row, col, embedding_data, *(float*)(&embedding_data), reference_data, *(float*)(&reference_data));
          fflush(stdout);
