...
void host_get_random_node_ids(void* nodes, int64_t node_count, int64_t range, bool unique) {
  ...
  if (!unique) {
    ...
    std::vector<DataType> tmp_array_out(range);
    for (int64_t i = 0; i < range; i++) {
      tmp_array_in[i] = i;
    }

    std::sample(tmp_array_in.begin(), tmp_array_in.end(), tmp_array_out.begin(), node_count, gen);

    for (int64_t i = 0; i < node_count; i++) {
      nodes_ptr[i] = tmp_array_out[i];
    }
  }
}
...
template <typename DataType>
void insert_nodes_to_append_unique_hash_table(std::unordered_map<DataType, int>& append_unique_hash_table, DataType* node_ptr, int64_t node_count, int* unique_count_ptr, bool target)
{
    if (target) {
        for (int64_t i = 0; i < node_count; i++) {
                            ...
                            wholememory_array_description_t output_neighbor_raw_to_unique_desc)
{
  ...
  *host_total_unique_count      = unique_count;
  ...
  for (auto iter = append_unique_hash_table.begin(); iter != append_unique_hash_table.end(); iter++){
