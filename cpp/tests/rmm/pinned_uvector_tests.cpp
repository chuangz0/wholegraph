...
template class rmm::pinned_uvector<int32_t>;
...
{
  rmm::pinned_uvector<TypeParam> vec(128, this->stream());
  EXPECT_EQ(vec.memory_resource(), rmm::mr::get_pinned_resource());
}

TYPED_TEST(TypedUVectorTest, ZeroSizeConstructor)
{
  rmm::pinned_uvector<TypeParam> vec(0, this->stream());
...
TYPED_TEST(TypedUVectorTest, NonZeroSizeConstructor)
{
  ...
  rmm::pinned_uvector<TypeParam> vec(size, this->stream());
...
TYPED_TEST(TypedUVectorTest, CopyConstructor)
{
  auto const size{12345};
  rmm::pinned_uvector<TypeParam> vec(size, this->stream());
  rmm::pinned_uvector<TypeParam> uv_copy(vec, this->stream());
...
TYPED_TEST(TypedUVectorTest, ResizeSmaller)
{
  ...
  rmm::pinned_uvector<TypeParam> vec(original_size, this->stream());
...
TYPED_TEST(TypedUVectorTest, Release)
{
  ...
  rmm::pinned_buffer storage = vec.release();
