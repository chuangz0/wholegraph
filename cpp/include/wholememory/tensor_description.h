...
typedef struct wholememory_tensor_description_ {
  int64_t sizes[WHOLEMEMORY_MAX_TENSOR_DIM];                /* size of each dimension of the tensor, in number of elements */
 ...
 * @param stride : stride of last dimension(slower changed dimension), stride is counted in number of elements
 * @param storage_offset : storage offset in number of elements
...
/*!
 * Initialize wholememory_tensor_description_t, set sizes and strides to all ones, and set storage_offset to 0,
 * set dtype to WHOLEMEMORY_DT_UNKNOWN, set dim to 0.
