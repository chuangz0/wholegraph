...
wm_thrust_allocator::~wm_thrust_allocator() {
  deallocate_all();
}
...
wm_thrust_allocator::value_type* wm_thrust_allocator::allocate(std::ptrdiff_t mem_size) {
  ...
  auto *memory_context = fns->temporary_fns.create_memory_context_fn(fns->temporary_fns.global_context);
  ...
  auto *ptr = static_cast<value_type *>(fns->temporary_fns.device_malloc_fn(&tensor_description,
                                                                            memory_context,
                                                                            fns->temporary_fns.global_context));
