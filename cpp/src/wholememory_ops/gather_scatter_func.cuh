 ...
 */
inline int determine_wholememory_alignment_elt_count(wholememory_matrix_description_t embedding_desc) {
  ...
  for (; alignment > 1; alignment /= 2) {
    if (embedding_desc.storage_offset % alignment == 0 && embedding_desc.sizes[0] % alignment == 0 && embedding_desc.stride % alignment == 0)
      break;
  }
  return alignment;
}
 ...
 * @return : Alignment that can be used, in element count.
 */
inline int determine_memory_alignment_elt_count(const void* ptr, wholememory_matrix_description_t memory_desc) {
  ...
  for (; alignment > 1; alignment /= 2) {
    if (int_ptr % alignment == 0 && memory_desc.sizes[0] % alignment == 0 && memory_desc.stride % alignment == 0)
      break;
  }
  return alignment;
}
