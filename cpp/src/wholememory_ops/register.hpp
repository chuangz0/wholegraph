...
struct one_wmt_hash : public std::unary_function<wholememory_dtype_t, std::size_t> {
  inline std::size_t operator()(const wholememory_dtype_t &k) const { return static_cast<size_t>(k); }
};
...
struct three_wmt_hash
    : public std::unary_function<std::tuple<wholememory_dtype_t, wholememory_dtype_t, wholememory_dtype_t>,
                                 std::size_t> {
  inline std::size_t
  operator()(const std::tuple<wholememory_dtype_t, wholememory_dtype_t, wholememory_dtype_t> &k) const {
...
template<>
inline wholememory_dtype_t get_wholememory_dtype<int8_t>() { return WHOLEMEMORY_DT_INT8; }
template<>
inline wholememory_dtype_t get_wholememory_dtype<int16_t>() { return WHOLEMEMORY_DT_INT16; }
template<>
inline wholememory_dtype_t get_wholememory_dtype<int32_t>() { return WHOLEMEMORY_DT_INT; }
template<>
inline wholememory_dtype_t get_wholememory_dtype<int64_t>() { return WHOLEMEMORY_DT_INT64; }
template<>
inline wholememory_dtype_t get_wholememory_dtype<__half>() { return WHOLEMEMORY_DT_HALF; }
template<>
inline wholememory_dtype_t get_wholememory_dtype<__nv_bfloat16>() { return WHOLEMEMORY_DT_BF16; }
template<>
inline wholememory_dtype_t get_wholememory_dtype<float>() { return WHOLEMEMORY_DT_FLOAT; }
template<>
inline wholememory_dtype_t get_wholememory_dtype<double>() { return WHOLEMEMORY_DT_DOUBLE; }


#define VEC_SINT3264 std::vector<wholememory_dtype_t>({WHOLEMEMORY_DT_INT, WHOLEMEMORY_DT_INT64})
#define VEC_ALLSINT \
  std::vector<wholememory_dtype_t>({WHOLEMEMORY_DT_INT8, WHOLEMEMORY_DT_INT16, WHOLEMEMORY_DT_INT, WHOLEMEMORY_DT_INT64})
...
#define VEC_BF16_HALF_FLOAT std::vector<wholememory_dtype_t>({WHOLEMEMORY_DT_BF16, WHOLEMEMORY_DT_HALF, WHOLEMEMORY_DT_FLOAT})
#define VEC_HALF_FLOAT_DOUBLE std::vector<wholememory_dtype_t>({WHOLEMEMORY_DT_HALF, WHOLEMEMORY_DT_FLOAT, WHOLEMEMORY_DT_DOUBLE})
#define VEC_ALLFLOAT std::vector<wholememory_dtype_t>({WHOLEMEMORY_DT_BF16, WHOLEMEMORY_DT_HALF, WHOLEMEMORY_DT_FLOAT, WHOLEMEMORY_DT_DOUBLE})
#define VEC_ALLSINT_ALLFLOAT std::vector<wholememory_dtype_t>({WHOLEMEMORY_DT_INT8, WHOLEMEMORY_DT_INT16, WHOLEMEMORY_DT_INT, WHOLEMEMORY_DT_INT64, WHOLEMEMORY_DT_BF16, WHOLEMEMORY_DT_HALF, WHOLEMEMORY_DT_FLOAT, WHOLEMEMORY_DT_DOUBLE})
...
#define REGISTER_DISPATCH_ONE_TYPE(NAME, TEMPFUNC_NAME, ARG0_SET)                                                      \
  static std::unordered_map<wholememory_dtype_t, decltype(&TEMPFUNC_NAME<int>), wholememory_ops::one_wmt_hash>         \
      *NAME##_dispatch1_map = nullptr;                                                                                 \
  ...
  __attribute__((constructor)) static void Register##NAME##Map1Func() {                                                \
    NAME##_dispatch1_map =                                                                                             \
        new std::unordered_map<wholememory_dtype_t, decltype(&TEMPFUNC_NAME<int>), wholememory_ops::one_wmt_hash>();   \
      ...
      switch (arg0_type) {                                                                                             \
        ...
        default : {                                                                                                    \
          WHOLEMEMORY_FAIL_NOTHROW("dispatch with type=%d for function %s failed.",                                    \
              static_cast<int>(arg0_type), #TEMPFUNC_NAME);                                                            \
          break;                                                                                                       \
        }                                                                                                              \
      }                                                                                                                \
    }                                                                                                                  \
...
#define REGISTER_DISPATCH_TWO_TYPES(NAME, TEMPFUNC_NAME, ARG0_SET, ARG1_SET)                                           \
  static std::unordered_map<std::tuple<wholememory_dtype_t, wholememory_dtype_t>,                                      \
                            ...
                            wholememory_ops::two_wmt_hash> *NAME##_dispatch2_map =                                     \
      nullptr;                                                                                                         \
  ...
  void Register##NAME##Map2FuncHelper1() {                                                                             \
    ...
    for (auto arg0_type : arg0_types) {                                                                                \
      switch (arg0_type) {                                                                                             \
        CASES_##ARG0_SET(Register##NAME##Map2FuncHelper0, T1)                                                          \
        default : {                                                                                                    \
      ...
      switch (arg1_type) {                                                                                             \
        ...
        default : {                                                                                                    \
          WHOLEMEMORY_FAIL_NOTHROW("dispatch with type1=%d for function %s failed.",                                   \
              static_cast<int>(arg1_type), #TEMPFUNC_NAME);                                                            \
          break;                                                                                                       \
        }                                                                                                              \
      }                                                                                                                \
    }                                                                                                                  \
...
#define REGISTER_DISPATCH_THREE_TYPES(NAME, TEMPFUNC_NAME, ARG0_SET, ARG1_SET, ARG2_SET)                               \
  static std::unordered_map<std::tuple<wholememory_dtype_t, wholememory_dtype_t, wholememory_dtype_t>,                 \
                            decltype(&TEMPFUNC_NAME<int, int, int>),                                                   \
                            wholememory_ops::three_wmt_hash> *NAME##_dispatch3_map =                                   \
      nullptr;                                                                                                         \
  template<typename T0, typename T1, typename T2>                                                                      \
  void Register##NAME##Map3FuncHelper0() {                                                                             \
    auto key =                                                                                                         \
        std::make_tuple(get_wholememory_dtype<T0>(), get_wholememory_dtype<T1>(), get_wholememory_dtype<T2>());        \
  ...
  void Register##NAME##Map3FuncHelper1() {                                                                             \
    ...
    for (auto arg0_type : arg0_types) {                                                                                \
      switch (arg0_type) {                                                                                             \
        CASES_##ARG0_SET(Register##NAME##Map3FuncHelper0, T1, T2)                                                      \
        default : {                                                                                                    \
          WHOLEMEMORY_FAIL_NOTHROW("dispatch with type0=%d for function %s failed.",                                   \
              static_cast<int>(arg0_type), #TEMPFUNC_NAME);                                                            \
          break;                                                                                                       \
  ...
  __attribute__((constructor)) static void Register##NAME##Map3Func() {                                                \
    NAME##_dispatch3_map =                                                                                             \
        new std::unordered_map<std::tuple<wholememory_dtype_t, wholememory_dtype_t, wholememory_dtype_t>,              \
                               decltype(&TEMPFUNC_NAME<int, int, int>),                                                \
      ...
      switch (arg2_type) {                                                                                             \
        ...
        default : {                                                                                                    \
          WHOLEMEMORY_FAIL_NOTHROW("dispatch with type2=%d for function %s failed.",                                   \
              static_cast<int>(arg2_type), #TEMPFUNC_NAME);                                                            \
          break;                                                                                                       \
        }                                                                                                              \
      }                                                                                                                \
    }                                                                                                                  \
