...
class wholememory_impl {
 public:
  ...
  [[nodiscard]] wholememory_memory_type_t get_type() const {
    return type_;
  }
  [[nodiscard]] wholememory_memory_location_t get_location() const {
    return location_;
  }
  [[nodiscard]] wholememory_comm_t get_comm() const {
    return comm_;
  }
 ...
 protected:
  // In WholeMemory, memory is first allocated by one or all ranks, and then partition the whole memory to each rank.
  // Each rank can direct access its partition of memory and is response for memory operations on that.
  // In some WholeMemory types like CONTINUOUS or CHUNKED, memory of other ranks may also be mapped to current rank.
  // So current rank may access the whole memory, but in a parallel loading case, it is not its response to process
  // memory other than the partition of memory determined by memory partition strategy.
  //
  // Memory partitioning is decoupled with memory allocation, memory allocation may have different strategies.
  //
  // The following 3 functions are for memory allocation strategies
  // first rank responsible for all memory allocation, continuous or chunked host shared memory may use this mode.
  void first_rank_allocate_all_strategy();
  // each rank allocate exactly the same size, chunked device memory or nccl memory may use this mode.
  void each_rank_same_chunk_strategy();
  // each rank allocate a multiple of pages, and map the whole memory by page, continuous device memory use this mode.
  ...
  // And local_mem_offset_ can also be got by rank_mem_stride_ * rank for ranks with local_mem_size_ != 0
  // That means for a valid memory offset position, offset / rank_mem_stride_ can be used to get the rank which is
  // responsible for it.
...
wholememory_handle_t wholememory_get_handle(const void *ptr) {
  ...
  if (wm_h->impl->contains_pointer(ptr)) {
    return wm_h;
  }
  return nullptr;
}
...
static void register_wholememory_vma_range_locked(const void *ptr, size_t mem_block_size, wholememory_handle_t wm_h) {
  ...
  if (it1 != wholememory_vma_map.begin()) {
    --it1;
    WHOLEMEMORY_CHECK(reinterpret_cast<uint64_t>(it1->second.start_ptr) + it1->second.mem_block_size <= int_start_ptr);
  }
 ...
 protected:
  void register_private_memory() {
    ...
    if (alloc_strategy_.local_alloc_size > 0) {
      register_wholememory_vma_range_locked(no_ipc_handle_.local_alloc_mem_ptr,
  ...
  void unregister_private_memory() noexcept {
    std::unique_lock<std::mutex> vma_lock(wholememory_vma_mu);
    if (alloc_strategy_.local_alloc_size > 0) {
      unregister_wholememory_vma_range_locked(no_ipc_handle_.local_alloc_mem_ptr,
                                              alloc_strategy_.local_alloc_size,
                                              handle_);
    }
  }
...
class global_mapped_host_wholememory_impl : public wholememory_impl {
 ...
 protected:
  void register_host_memory() {
    std::unique_lock<std::mutex> vma_lock(wholememory_vma_mu);
    register_wholememory_vma_range_locked(shared_host_handle_.shared_host_memory_ptr, total_size_, handle_);
  }
  void unregister_host_memory() noexcept {
    std::unique_lock<std::mutex> vma_lock(wholememory_vma_mu);
    unregister_wholememory_vma_range_locked(shared_host_handle_.shared_host_memory_ptr, total_size_, handle_);
  }
    ...
    if (comm_->world_rank == 0) {
      ...
      if (shm_fd < 0) {
        WHOLEMEMORY_FATAL("Rank=%d open host shared memory from file %s failed.",
                          comm_->world_rank, shm_full_path.c_str());
      }
    }
    communicator_barrier(comm_);
...
class continuous_device_wholememory_impl : public wholememory_impl {
 ...
 protected:
  void register_continuous_device_memory() {
    std::unique_lock<std::mutex> vma_lock(wholememory_vma_mu);
    register_wholememory_vma_range_locked(cu_alloc_handle_.mapped_whole_memory, total_size_, handle_);
  }
  void unregister_continuous_device_memory() noexcept {
    std::unique_lock<std::mutex> vma_lock(wholememory_vma_mu);
    unregister_wholememory_vma_range_locked(cu_alloc_handle_.mapped_whole_memory, total_size_, handle_);
  }
  ...
  static int ipc_open_socket(const std::string &name) {
    ...
    if (name.length() >= sizeof(skt_addr.sun_path)) {
      WHOLEMEMORY_FATAL("IPC socket path length (%zu) larger than sockaddr_un.sun_path length (%lu), full_path: %s",
                        name.length(), sizeof(skt_addr.sun_path), name.c_str());
    }
  ...
  [[nodiscard]] std::string get_recver_fd_name(int src_id) const {
    std::string name = get_temporary_directory_path(comm_);
    name.append("/recver_").append(std::to_string(src_id)).append("_to_").append(std::to_string(comm_->world_rank)).append(
        ".sock");
    return name;
  }
  [[nodiscard]] std::string get_target_recver_fd_name(int dst_id) const {
    std::string name = get_temporary_directory_path(comm_);
    name.append("/recver_").append(std::to_string(comm_->world_rank)).append("_to_").append(std::to_string(dst_id)).append(
        ".sock");
    return name;
  }
  ...
  static void ipc_send_sharable_handle(int sock_fd,
    ...
    if (send_result <= 0) {
      WHOLEMEMORY_FATAL("IPC failure: Sending data over socket failed send_result=%ld.", send_result);
    }
  }
  static ipc_sharable_cu_handle ipc_recv_sharable_handle(int recv_sock_fd) {
    ...
    if (((cmptr = CMSG_FIRSTHDR(&message_header)) != nullptr) && (cmptr->cmsg_len == CMSG_LEN(sizeof(int)))) {
  ...
  void create_and_map_driver_device_memory() {
    ...
    if (alloc_strategy_.local_alloc_size > 0) {
      cu_alloc_handle_.local_cu_handle = create_cu_mem(alloc_strategy_.local_alloc_size, comm_->dev_id);
    }
    ...
    exchange_driver_device_memory_handles(&recv_ipc_sharable_cu_handles, &send_ipc_sharable_cu_handles);
    close_unix_domain_sockets();
    map_driver_device_memory_handles(&recv_ipc_sharable_cu_handles);
    communicator_barrier(comm_);
    local_partition_memory_pointer_ =
        static_cast<char *>(cu_alloc_handle_.mapped_whole_memory) + rank_partition_strategy_.local_mem_offset;
  }
...
void wholememory_impl::generate_rank_partition_strategy() {
  ...
  size_t rank_data_slot_end = std::min((comm_->world_rank + 1) * data_slot_per_rank, data_slot_count);
...
void wholememory_impl::first_rank_allocate_all_strategy() {
  ...
  alloc_strategy_.local_alloc_size = (comm_->world_rank == 0) ? alloc_strategy_.total_alloc_size : 0;
...
void wholememory_impl::each_rank_same_chunk_strategy() {
  ...
  if (total_size_ > HUGE_PAGE_THRESHOLD) {
    alloc_strategy_.local_alloc_size = raft::round_up_safe(alloc_strategy_.local_alloc_size, HUGE_PAGE_SIZE);
...
wholememory_error_code_t create_wholememory(wholememory_handle_t *wholememory_handle_ptr,
  ...
  try {
    ...
    *wholememory_handle_ptr = nullptr;
    ...
    comm->wholememory_map.insert(std::pair<int, wholememory_handle_t>(whole_memory_handle->handle_id,
                                                                      whole_memory_handle));
...
wholememory_error_code_t destroy_wholememory_with_comm_locked(wholememory_handle_t wholememory_handle) noexcept {
  try {
...
wholememory_error_code_t determine_partition_plan(size_t *size_per_rank,
  ...
  if (total_size % data_granularity != 0) {
    return WHOLEMEMORY_INVALID_VALUE;
  }
  if (size_per_rank == nullptr) {
    return WHOLEMEMORY_INVALID_INPUT;
  }
...
size_t determine_entry_partition_plan(size_t total_entry_count,
                                      int world_size) noexcept {

  return raft::div_rounding_up_safe<size_t>(total_entry_count, world_size);
}

wholememory_error_code_t get_partition_plan_from_handle(size_t *size_per_rank,
                                                        wholememory_handle_t wholememory_handle) noexcept {
  if (wholememory_handle == nullptr || wholememory_handle->impl == nullptr) {
    return WHOLEMEMORY_INVALID_INPUT;
  }
