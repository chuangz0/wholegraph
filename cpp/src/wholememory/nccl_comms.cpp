...
nccl_comms::nccl_comms(ncclComm_t nccl_comm,
                       int num_ranks,
                       int rank,
                       cudaStream_t rmm_stream)
    : nccl_comm_(nccl_comm),
      rmm_stream_(rmm_stream),
...
#endif
default:
    WHOLEMEMORY_FAIL("get_nccl_datatype_size");
    }
...
void nccl_comms::host_allreduce(const void *sendbuff,
                                void *recvbuff,
                                size_t count,
                                ncclDataType_t datatype,
  ...
  for (size_t offset = 0; offset < count; offset += max_elt_count) {
    ...
    RAFT_NCCL_TRY(ncclAllReduce(
        ...
        rmm_stream_));
    CUDA_CHECK(cudaStreamSynchronize(rmm_stream_));
    std::memcpy(static_cast<char *>(recvbuff) + datatype_size * offset, host_recv_buffer_, elt_count * datatype_size);
  }
}

void nccl_comms::bcast(void *buff,
                       size_t count,
                       ncclDataType_t datatype,
                       int root,
                       cudaStream_t stream) const {
  RAFT_NCCL_TRY(
      ncclBroadcast(buff, buff, count, datatype, root, nccl_comm_, stream));
}
...
void nccl_comms::bcast(const void *sendbuff,
  ...
  RAFT_NCCL_TRY(ncclBroadcast(
      sendbuff, recvbuff, count, datatype, root, nccl_comm_, stream));
}

void nccl_comms::host_bcast(const void *sendbuff,
                            void *recvbuff,
                            size_t count,
                            ncclDataType_t datatype,
                            int root) const {
...
void nccl_comms::allgather(const void *sendbuff,
  ...
  RAFT_NCCL_TRY(ncclAllGather(
      sendbuff, recvbuff, sendcount, datatype, nccl_comm_, stream));
}
...
void nccl_comms::allgatherv(const void *sendbuf,
  ...
  WHOLEMEMORY_EXPECTS(num_ranks_ <= 2048,
...
void nccl_comms::gather(const void *sendbuff,
  ...
  RAFT_NCCL_TRY(
      ncclSend(sendbuff, sendcount, datatype, root, nccl_comm_, stream));
  RAFT_NCCL_TRY(ncclGroupEnd());
}

void nccl_comms::host_gather(const void *sendbuff,
                             void *recvbuff,
                             size_t sendcount,
                             ncclDataType_t datatype,
                             int root) const {
...
void nccl_comms::host_alltoall(const void *sendbuff,
  ...
  for (size_t offset = 0; offset < sendcount; offset += max_elt_count) {
    size_t elt_count = (sendcount - offset > max_elt_count) ? max_elt_count : sendcount - offset;
    for (int i = 0; i < num_ranks_; i++) {
      std::memcpy(host_send_buffer_ + i * elt_count * datatype_size,
...
void nccl_comms::device_send(const void *send_buf, size_t send_size, int dest, cudaStream_t stream) const {
  RAFT_NCCL_TRY(ncclSend(send_buf, send_size, ncclUint8, dest, nccl_comm_, stream));
}

// if a thread is sending & receiving at the same time, use device_sendrecv to avoid deadlock
void nccl_comms::device_recv(void *recv_buf, size_t recv_size, int source, cudaStream_t stream) const {
