...
namespace rmm {
/**
 * @file pinned_buffer.hpp
 * @brief RAII construct for pinned memory allocation
 *
 * This class allocates untyped and *uninitialized* pinned memory using a
 * `pinned_memory_resource`. If not explicitly specified, the memory resource
 * returned from `get_pinned_resource()` is used.
 *
 * @note Unlike `std::vector` or `thrust::host_vector`, the pinned host memory
 * allocated by a `pinned_buffer` is uninitialized. Therefore, it is undefined
 * behavior to read the contents of `data()` before first initializing it.
 *
 * Examples:
 * ```
 * //Allocates at least 100 bytes of pinned memory using the default memory
 * //resource and default stream.size
 * pinned_buffer buff(100);
 *
 * // allocates at least 100 bytes using the custom memory resource and
 * // specified stream
 * custom_memory_resource mr;
 * cuda_stream_view stream = cuda_stream_view{};
 * pinned_buffer custom_buff(100, stream, &mr);
 *
 * // deep copies `buff` into a new pinned buffer using the specified stream
 * pinned_buffer buff_copy(buff, stream);
 *
 * // moves the memory in `from_buff` to `to_buff`. Deallocates previously allocated
 * // to_buff memory on `to_buff.stream()`.
 * pinned_buffer to_buff(std::move(from_buff));
 *
 * // shallow copies `buff` into a new pinned_buffer, `buff` is now empty
 * pinned_buffer buff_move(std::move(buff));
 *
 * // Default construction. Buffer is empty
 * pinned_buffer buff_default{};
 *
...
class pinned_buffer {
 public:
  ...
  pinned_buffer(pinned_buffer const& other) = delete;
  pinned_buffer& operator=(pinned_buffer const& other) = delete;

  /**
   * @brief Default constructor creates an empty `pinned_buffer`
   */
  // Note: we cannot use `pinned_buffer() = default;` because nvcc implicitly adds
  ...
  pinned_buffer() : _mr{rmm::mr::get_pinned_resource()} {}

  /**
   * @brief Constructs a new pinned buffer of `size` uninitialized bytes
   *
   * @throws rmm::bad_alloc If allocation fails.
   *
   * @param size Size in bytes to allocate in pinned memory.
   * @param stream CUDA stream on which memory may be allocated if the memory
   * resource supports streams.
   * @param mr Memory resource to use for the pinned memory allocation.
   */
  explicit pinned_buffer(std::size_t size,
                         cuda_stream_view stream,
                         mr::pinned_memory_resource* mr = mr::get_pinned_resource())
  ...
  /**
   * @brief Construct a new pinned buffer by copying from a raw pointer to an existing host or
   ...
   * @param mr Memory resource to use for the pinned memory allocation
   */
  pinned_buffer(void const* source_data,
                std::size_t size,
                cuda_stream_view stream,
                mr::pinned_memory_resource* mr = mr::get_pinned_resource())
      : _stream{stream}, _mr{mr}
  ...
  /**
   * @brief Construct a new `pinned_buffer` by deep copying the contents of
   * another `pinned_buffer`, optionally using the specified stream and memory
   * resource.
   *
   * @note Only copies `other.size()` bytes from `other`, i.e., if
   *`other.size() != other.capacity()`, then the size and capacity of the newly
   * constructed `pinned_buffer` will be equal to `other.size()`.
   *
   ...
   *
   * @param other The `pinned_buffer` whose contents will be copied
   * @param stream The stream to use for the allocation and copy
   * @param mr The resource to use for allocating the new `pinned_buffer`
   */
  pinned_buffer(pinned_buffer const& other,
                cuda_stream_view stream,
                rmm::mr::pinned_memory_resource* mr = rmm::mr::get_pinned_resource())
      : pinned_buffer{other.data(), other.size(), stream, mr}
  {
  }

  /**
   * @brief Constructs a new `pinned_buffer` by moving the contents of another
   * `pinned_buffer` into the newly constructed one.
   *
   * After the new `pinned_buffer` is constructed, `other` is modified to be a
   * valid, empty `pinned_buffer`, i.e., `data()` returns `nullptr`, and
   * `size()` and `capacity()` are zero.
   *
   * @throws Nothing
   *
   * @param other The `pinned_buffer` whose contents will be moved into the
   ...
   *
   * This `pinned_buffer`'s current pinned memory allocation will be deallocated
   * on `stream()`.
   *
   ...
   *
   * @param other The `pinned_buffer` whose contents will be moved.
   */
  ...
  /**
   * @brief Destroy the pinned buffer object
   *
   * @note If the memory resource supports streams, this destructor deallocates
   * using the stream most recently passed to any of this pinned buffer's
   * methods.
   */
  ...
  /**
   * @brief Increase the capacity of the pinned memory allocation
   *
  ...
  void reserve(std::size_t new_capacity, cuda_stream_view stream)
  {
    ...
    if (new_capacity > capacity()) {
      auto tmp            = pinned_buffer{new_capacity, stream, _mr};
  ...
  /**
   * @brief Resize the pinned memory allocation
   *
   ...
   * `capacity()` remains the actual size of the pinned memory allocation.
   *
  ...
  void resize(std::size_t new_size, cuda_stream_view stream)
  {
    ...
    if (new_size <= capacity()) {
      ...
      auto tmp = pinned_buffer{new_size, stream, _mr};
      RMM_CUDA_TRY(cudaMemcpyAsync(tmp.data(), data(), size(), cudaMemcpyDefault, stream.value()));
      *this = std::move(tmp);
    }
  }
   ...
   *
   * Reallocates and copies on stream `stream` the contents of the pinned memory
  ...
  void shrink_to_fit(cuda_stream_view stream)
  {
    ...
    if (size() != capacity()) {
      ...
      // with self. The temporary `pinned_buffer` will hold the old contents
      // which will then be destroyed
      auto tmp = pinned_buffer{*this, stream, _mr};
      std::swap(tmp, *this);
    }
  }

  /**
   * @brief Returns raw pointer to underlying pinned memory allocation
   */
   ...
   *
   * If `is_empty() == true`, the `pinned_buffer` may still hold an allocation
   * if `capacity() > 0`.
   *
   */
  ...
  /**
   * @brief Returns actual size in bytes of pinned memory allocation.
   *
   * The invariant `size() <= capacity()` holds.
   */
   ...
   *
   * If no other rmm::pinned_buffer method that allocates memory is called
   ...
   * deallocate the pinned memory
   */
  [[nodiscard]] mr::pinned_memory_resource* memory_resource() const noexcept { return _mr; }

 private:
  void* _data{nullptr};        ///< Pointer to pinned memory allocation
  std::size_t _size{};         ///< Requested size of the pinned memory allocation
  std::size_t _capacity{};     ///< The actual size of the pinned memory allocation
  cuda_stream_view _stream{};  ///< Stream to use for pinned memory deallocation
  mr::pinned_memory_resource* _mr{
      mr::get_pinned_resource()};  ///< The memory resource used to
  ///< allocate/deallocate pinned memory
  ...
  void allocate_async(std::size_t bytes)
  {
    _size     = bytes;
    _capacity = bytes;
    _data     = (bytes > 0) ? memory_resource()->allocate(bytes) : nullptr;
  }

  /**
   * @brief Deallocate any memory held by this `pinned_buffer` and clear the
  ...
  {
    if (capacity() > 0) { memory_resource()->deallocate(data(), capacity()); }
    _size     = 0;
    _capacity = 0;
    _data     = nullptr;
  }
